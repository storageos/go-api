/*
StorageOS API

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: 2.5.0
Contact: info@storageos.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package api

import (
	"encoding/json"
)

// AttachNFSVolumeData struct for AttachNFSVolumeData
type AttachNFSVolumeData struct {
	// An opaque representation of an entity version at the time it was obtained from the API. All operations that mutate the entity must include this version field in the request unchanged. The format of this type is undefined and may change but the defined properties will not change. 
	Version *string `json:"version,omitempty"`
}

// NewAttachNFSVolumeData instantiates a new AttachNFSVolumeData object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewAttachNFSVolumeData() *AttachNFSVolumeData {
	this := AttachNFSVolumeData{}
	return &this
}

// NewAttachNFSVolumeDataWithDefaults instantiates a new AttachNFSVolumeData object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewAttachNFSVolumeDataWithDefaults() *AttachNFSVolumeData {
	this := AttachNFSVolumeData{}
	return &this
}

// GetVersion returns the Version field value if set, zero value otherwise.
func (o *AttachNFSVolumeData) GetVersion() string {
	if o == nil || o.Version == nil {
		var ret string
		return ret
	}
	return *o.Version
}

// GetVersionOk returns a tuple with the Version field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AttachNFSVolumeData) GetVersionOk() (*string, bool) {
	if o == nil || o.Version == nil {
		return nil, false
	}
	return o.Version, true
}

// HasVersion returns a boolean if a field has been set.
func (o *AttachNFSVolumeData) HasVersion() bool {
	if o != nil && o.Version != nil {
		return true
	}

	return false
}

// SetVersion gets a reference to the given string and assigns it to the Version field.
func (o *AttachNFSVolumeData) SetVersion(v string) {
	o.Version = &v
}

func (o AttachNFSVolumeData) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if o.Version != nil {
		toSerialize["version"] = o.Version
	}
	return json.Marshal(toSerialize)
}

type NullableAttachNFSVolumeData struct {
	value *AttachNFSVolumeData
	isSet bool
}

func (v NullableAttachNFSVolumeData) Get() *AttachNFSVolumeData {
	return v.value
}

func (v *NullableAttachNFSVolumeData) Set(val *AttachNFSVolumeData) {
	v.value = val
	v.isSet = true
}

func (v NullableAttachNFSVolumeData) IsSet() bool {
	return v.isSet
}

func (v *NullableAttachNFSVolumeData) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableAttachNFSVolumeData(val *AttachNFSVolumeData) *NullableAttachNFSVolumeData {
	return &NullableAttachNFSVolumeData{value: val, isSet: true}
}

func (v NullableAttachNFSVolumeData) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableAttachNFSVolumeData) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


