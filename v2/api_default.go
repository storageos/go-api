...
// SetLogLevelOpts Optional parameters for the method 'SetLogLevel'
type SetLogLevelOpts struct {
    IgnoreVersion optional.Bool
}

/*
SetLogLevel Update the cluster's log level
Updates the cluster&#39;s log level at runtime (affecting all nodes) and persists the new log level across cluster restarts
 * @param ctx _context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 * @param setLogLevelData
 * @param optional nil or *SetLogLevelOpts - Optional Parameters:
 * @param "IgnoreVersion" (optional.Bool) -  If set to true this value indicates that the user wants to ignore entity version constraints, thereby \"forcing\" the operation. 
*/
func (a *DefaultApiService) SetLogLevel(ctx _context.Context, setLogLevelData SetLogLevelData, localVarOptionals *SetLogLevelOpts) (*_nethttp.Response, error) {
	var (
		localVarHTTPMethod   = _nethttp.MethodPut
		localVarPostBody     interface{}
		localVarFormFileName string
	...
	// create path and map variables
	localVarPath := a.client.cfg.BasePath + "/cluster/log-level"
	localVarHeaderParams := make(map[string]string)
	localVarQueryParams := _neturl.Values{}
	localVarFormParams := _neturl.Values{}
	...
	// body params
	localVarPostBody = &setLogLevelData
	r, err := a.client.prepareRequest(ctx, localVarPath, localVarHTTPMethod, localVarPostBody, localVarHeaderParams, localVarQueryParams, localVarFormParams, localVarFormFileName, localVarFileName, localVarFileBytes)
	if err != nil {
		return nil, err
	}
